    ...
    <>
      <div className={`${className} min-w-[18.75rem] rounded-lg shadow-md`}>
        <Link
          ...
          className={
            "rounded-t-lg w-full h-[17.5rem] bg-white flex items-center justify-center p-4"
          }
        >
          <img
            src={image}
            alt={name}
            className={"block my-auto w-full h-full object-cover"}
          />
        </Link>
        <div className={"p-4 py-4 rounded-b-lg"}>
          <div className={"flex flex-col gap-1 w-fit"}>
            <p className={"text-sm text-gray-8"}>{category}</p>
            <Link to={`/product/${productId}`} reloadDocument className={""}>
              {name}
            </Link>
